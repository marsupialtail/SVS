...
class TonyConvGradOp : public OpKernel {
 public:
  ...
  void Compute(OpKernelContext* context) override {
    ...
    const int input_channels = input_shape.dim_size(0);
    const int batch_size = input_shape.dim_size(3);
    const int input_height = input_shape.dim_size(1);
    const int input_width = input_shape.dim_size(2);

    const int input_dims [4] = {input_channels,input_height,input_width,batch_size};
