...
nvcc -std=c++11 -c -o approx_kernel2.cu.o zero.cu.cc  ${TF_CFLAGS[@]} -D PROFILE=0,GOOGLE_CUDA=1 -x cu -m64 -Xptxas -O3 -Xcompiler -fPIC -O3 --generate-line-info -use_fast_math -arch=sm_61 -Xptxas="-v"
g++ -std=c++11 -shared -o approx_kernel1.so approx_kernel_NHWC.cc approx_kernel2.cu.o -D IMAGE_DIM=$1 ${TF_CFLAGS[@]} -fPIC -L$CUDA_HOME/lib64 -lcudart ${TF_LFLAGS[@]}
