...
nvcc -std=c++11 -c -o approx_kernel.cu.o approx_kernel.cu.cc  ${TF_CFLAGS[@]} -D PROFILE=0,GOOGLE_CUDA=1 -x cu -m64 -Xptxas -O3 -Xcompiler -fPIC -O3 --generate-line-info -use_fast_math -arch=sm_61 -Xptxas="-v"
g++ -std=c++11 -shared -o approx_kernel.so approx_kernel.cc approx_kernel.cu.o  ${TF_CFLAGS[@]} -fPIC -L$CUDA_HOME/lib64 -lcudart ${TF_LFLAGS[@]}
