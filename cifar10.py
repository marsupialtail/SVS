...
def inference(images):
    ...
    def tony_conv(input, filter):
        ...
        def grad(dy):
            ...
            def tony_grad():
                ...
                if padding == 'VALID':
                    NCHW_input = tf.transpose(input, perm=[0, 3, 1, 2])
                    ...
                    with tf.device('/gpu:0'):
                        return [
                            tf.nn.conv2d_backprop_input(input_sizes=tf.shape(input), filter=filter, out_backprop=dy,
                                                        strides=stride_list,
                                                        padding=padding, data_format=DF),
                            tf.transpose(approx_module.tony_conv_grad(NCHW_input, NCHW_dy, stride_list[2], filter_x, filter_y),
                                         perm=[2, 3, 1, 0])]
                else:
    ...
    with tf.variable_scope('conv1') as scope:
        kernel = _variable_with_weight_decay('weights',
                                             ...
                                             stddev=5e-2,
                                             wd=None)
        conv = tf.nn.conv2d(images, kernel,[1,1,1,1],'SAME',data_format=DF)
    ...
    if DF == "NHWC":
      pool1 = tf.nn.max_pool(conv1, ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1],
                           padding='SAME', name='pool1',data_format = DF)
    else:
    ...
    if DF == "NHWC":
      pool2 = tf.nn.max_pool(conv2, ksize=[1, 3, 3, 1],
                           strides=[1, 2, 2, 1], padding='SAME', name='pool2',data_format = DF)
    else:
      pool2 = tf.nn.max_pool(conv2, ksize=[1, 1, 3, 3],
                           strides=[1, 1, 2, 2], padding='SAME', name='pool2',data_format = DF)
