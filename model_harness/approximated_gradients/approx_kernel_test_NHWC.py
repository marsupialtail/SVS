...
NHWC_dy = tf.transpose(dy,perm=[0,2,3,1])

reshaped_dy = tf.reshape(NHWC_dy,[-1, NHWC_dy.shape[1] * NHWC_dy.shape[2], NHWC_dy.shape[3]])
...
row_idx = max_idx // NHWC_dy.get_shape().as_list()[1]
col_idx = max_idx % NHWC_dy.get_shape().as_list()[1]
...
NHWC_image = tf.pad(NHWC_image, [[0, 0], [1, 2], [1, 2], [0,0]])
#sums = tf.squeeze(scalings)

#start = time.time()
kernel_output = approx_module.tony_conv_grad_flex(NHWC_image,dy,stride=1,filter_x=filter_x,filter_y=filter_y)
